...
data$weight[] = NA
data$weight[] = ntop.keep(position.score, 4, 8)  
capital = 120000
data$weight[] = (capital / prices) * bt.exrem(data$weight)		
top4.d.keep8 = bt.run(data, type='share', trade.summary=T, capital=capital)
...
# Plot Strategy Metrics Side by Side
plotbt.strategy.sidebyside(top2.d.keep4, top4.d.keep6, top4.d.keep8, top6.d.keep8, perfromance.fn = 'engineering.returns.kpi')

################################
print(top2.d.keep4$trade, order("entry.date", decreasing=T))

################################
print(top4.d.keep6$trade, order("entry.date", decreasing=T))

################################
print(top4.d.keep8$trade, order("entry.date", decreasing=T))

################################
print(top6.d.keep8$trade, order("entry.date", decreasing=T))
