...
sym_gtaa_data <- function(strsym, prev_month=FALSE) {	
  ...
  ret14d <- calc_ret(m, idx, 14)
  ret30d <- calc_ret(m, idx, 30)
  ret365d <- calc_ret(m, idx, 365)
  ...
  res <- list(Sym=nm, Close=close, Prev=close1, Range=rng, Change=avg_ret,  R14d=ret14d, R30d=ret30d, RoC=RET,  OverMA20=gt_smaf, OverMA50=gt_smas, OverMA200=gt_smavs)
  return(res)
}
